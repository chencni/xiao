...
//富文本编辑器UEditor
        ...
        var ue = UE.getEditor('newsUeditor', {
            ...
            // UEDITOR_HOME_URL: "/myBlog/third/ueditor/"
        });
...
//分页导航twbsPagination
        ...
        $("[data-target='#newsModal']").click(
            function () {
                var headerHtml = $(this).children().first().html();
                var contentHtml = $(this).parent().parent().children(".hiddenCotent").html();
                $("#newsModal .modal-title").html(headerHtml);
                $("#newsContentModal").html(contentHtml);
                $("#newsContentModal img").addClass("img-responsive");//图片自适应
            }
        );
        //快速登录
        ...
        $("#sendNews").click(
                function () {
                    var newsImg = $("#newsImg").val()==""?$("#preview").attr("src"):$("#newsImg").val();//没选使用默认图片
                    var title = $("#title").val();
                    var detail = $("#detail").val();
                    var content = ue.getContent();
                    var url = "SendNewsAction";
                    var data = {
                        "newsImg": newsImg,
                        "title": title,
                        "detail": detail,
                    ...
                    $.post(url, "newsJson="+JSON.stringify(data), callbackFn);


                    function callbackFn(returnData, status, xhr) {
                        var saveRs = returnData.saveRs;
                        pageClick(null, 1);//跳到最后一页
                    };
                    //清空
                    $("#newsForm")[0].reset();
                    $("#preview").attr("src", "");
